...
$.fn.addSlider = function(values){
	...
	var timelineDataWidth = 107;	//Initial width to account for parent padding
	timelineData.children("ul").children("li").each(function () {
		timelineDataWidth += $(this).outerWidth();
	});
	timelineData.children("ul").children("li:last-of-type").append('<span class="end"/>')
	var decades = $(".decades");
	var fisheye = $("");
	...
	{
	var points = timelineData.children("ul").children("li").not(".pointInTime").length;
	var pointWidth = Math.floor((timeline.width() - 107 ) / points);
	timelineData.children("ul").children("li").not(".pointInTime").css("width", pointWidth+"px");
	}
